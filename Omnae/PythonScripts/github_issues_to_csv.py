...
def write_issues(r, csvout):
    ...
    for issue in r.json():
        if 'pull_request' not in issue:
            ...
            issue['title'] = issue['title'].encode('ascii', 'ignore').decode('ascii')
            issue['state'] = issue['state'].encode('ascii', 'ignore').decode('ascii')
            issue['html_url'] = issue['html_url'].encode('ascii', 'ignore').decode('ascii')
            date = issue['created_at'].split('T')[0].encode('ascii', 'ignore').decode('ascii')
            # Change the following line to write out additional fields			
            csvout.writerow([issue['user']['login'], issue['title'], issue['state'], date, issue['html_url']])
...
def get_issues(name):
    ...
    with open(csvfilename, 'w') as csvfile:
        ...
        csvout.writerow(['user', 'Title', 'State', 'Date', 'URL'])
