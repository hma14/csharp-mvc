...
using Omnae.SyncHobSpotUser.Auth0;
...
using Omnae.SyncHobSpotUser.Model;
...
namespace Omnae.SyncHobSpotUser.Service
{
    class UserSyncService : IDisposable
    {
        ...
        private IHubSpotClient hubSpotClient { get; }
        ...
        public UserSyncService(ILogger log)
        {
            ...
            //var hubSpotApikey = Environment.GetEnvironmentVariable("hubspot_APIKEY");
            HubSpotToken token = new HubSpotToken { AccessToken = accessToken };
            HubSpotClient = new HubSpotApi(token);


            //CompanySyncService = new CompanySyncService(log.ForContext<CompanySyncService>(), HubSpotClient);
            CompanySyncService = new CompanySyncService(log, HubSpotClient);
            ...
            Email.DefaultSender = new MailgunSender(
                Environment.GetEnvironmentVariable("Mailgun.Domain"), // Mailgun Domain
                Environment.GetEnvironmentVariable("Mailgun.APIKEY") // Mailgun API Key
            );


            //Db Conn
            OmnaeDbConnection = new SqlConnection(Environment.GetEnvironmentVariable("OmnaeDbContext"));

            //Configure API
            var baseUrl = Environment.GetEnvironmentVariable("hubspot_baseurl");
            ...
            hubspotHttpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + accessToken);
            //hubspotHttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
        }
        ...
        public async Task<bool> SyncAsync()
        {
            ...
            Log.LogInformation("Last HubspotIntegrationSyncControl info: {@Data}", lastCheck);

            var endDate = DateTime.Now.AddDays(1);

            //Log.LogInformation("The last user date created in Omnae Database is {0} and last updated was at {1}", lastUserCreatedInOmnaeDB, lastUserUpdatedInOmnaeDB);
            ...
            {
                Log.LogInformation("No new user was found in auth0 created after {0}.", lastCheck.LastCheckForNewAuthZeroUsers);
                //Log.LogInformation("No new user was found in auth0 created after {0} or updated after {1}", lastCheck.LastCheckForNewAuthZeroUsers, lastCheck.LastCheckForNewAuthZeroUsers);
                return false;
            }
        ...
        private async Task SendNewUserToCoreApi(List<OAuthUserData> mappedUsers)
        {
            ...
            foreach (var user in mappedUsers.Select(Map))
            {
                ...
                {
                    Log.LogError("User companyId is null for UserName: {0}.", user.UserName);
                    continue;
                }
        ...
        {
            Log.LogInformation("Start send the data to HubSpot.");
            ...
            Log.LogInformation("Ended send the data to HubSpot.");
        }
...
ORDER BY c.[Id] ASC", new { Ids = companiesIds });
        ...
        public async Task SendEmail(List<OAuthUserData> mapedUsers, string companyFile, string userFile, DateTime fromDate, DateTime toDate)
        {
            ...
            var toEmails = Environment.GetEnvironmentVariable("SendToEmails").Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(e => e.Trim()).ToList();
            ...
            Log.LogInformation("Email with integrations files send. Subject:{Subject}, Status:{Status}", subject, response.Successful);
        }

        private async Task SetHubspotParentChildRelations(List<OAuthUserData> oAuthUsers)
        {
            Log.LogInformation("Number of oAuthUsers: {count}", oAuthUsers.Count);
            var apiKey = Environment.GetEnvironmentVariable("hubspot_APIKEY");
            ...
            foreach (var oAuthUser in oAuthUsers)
            {
                ...
                var result = await hubspotHttpClient.PutAsJsonAsync($"/crm-associations/v1/associations", data);
                ...
                if (!result.IsSuccessStatusCode)
                {
                    ...
                    {
                        Log.LogWarning("The new Front-End API send a BadRequest or a Conflict. {StatusCode} - {Body}", result.StatusCode, body);
                        continue;
                    }
                    Log.LogError("The new Front-End API returned a error when creating Parent - Child relationship. {StatusCode} - {Body}", result.StatusCode, body);
                    result.EnsureSuccessStatusCode();
                    continue;
                }
                else
                {
                    Log.LogInformation("Is Success? {result}", result.IsSuccessStatusCode);
                }
            }
        }
